...
def image_to_binary(image, hue: float, margin: float, min_sat: float):
    ...
    sat_threshold = min_sat/255
    if lower_threshold < 0:
        binary_image = np.where(
            ((hue_channel >= 1 + lower_threshold) | (hue_channel <= upper_threshold))
            & (sat_channel > sat_threshold),
            1,
            0,
        )
    elif upper_threshold > 1:
        binary_image = np.where(
            ((hue_channel < upper_threshold - 1) | (hue_channel > lower_threshold))
            & (sat_channel > sat_threshold),
            1,
            0,
        )
        ...
        binary_image = np.where(
            ((hue_channel >= lower_threshold) & (hue_channel <= upper_threshold))
            & (sat_channel > sat_threshold),
            1,
            0,
        )
...
def detect_centroids(
    filepath, hue: float, margin: float, min_sat: float = 0, min_size: int = None
):
    ...
    binary_image = image_to_binary(image, hue, margin, min_sat)
